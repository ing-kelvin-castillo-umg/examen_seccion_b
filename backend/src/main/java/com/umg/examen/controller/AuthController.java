...
public class AuthController {
    ...
    @PostMapping("/refresh")
    @Operation(summary = "Renovar access token", description = "Valida un refresh token y retorna un nuevo access token")
    public ResponseEntity<ApiResponse<RefreshTokenResponse>> refresh(
            @Valid @RequestBody RefreshTokenRequest request) {
        RefreshTokenResponse refreshResponse = authService.refresh(request);
        return ResponseEntity.ok(ApiResponse.success("Access token renovado exitosamente", refreshResponse));
    }

    @PostMapping("/logout")
    @Operation(summary = "Cerrar sesion", description = "Revoca el refresh token de la sesion actual")
