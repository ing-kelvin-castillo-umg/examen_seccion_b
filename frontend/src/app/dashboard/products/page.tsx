...
export default function ProductsPage() {
  ...
  return (
    <div className="p-6 sm:p-10 space-y-8 max-w-7xl w-full mx-auto">
      ...
      {toast && (
        <div
          className={`fixed bottom-6 right-6 z-50 flex items-center gap-2.5 px-5 py-3.5 rounded-2xl shadow-neon-emerald border text-xs font-bold animate-in slide-in-from-bottom-5 duration-300 ${
            toast.type === "success"
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-brand-400 text-[11px] font-bold uppercase tracking-wider mb-1">
            <Boxes className="w-4 h-4 text-brand-400" />
            <span>Módulo de Inventario Centralizado</span>
          </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-2.5 rounded-xl border border-slate-800 bg-dark-900 hover:bg-dark-850 text-slate-300 transition-colors shadow-sm disabled:opacity-50"
          >
            <RefreshCw className={`w-4 h-4 ${loading ? "animate-spin text-brand-400" : ""}`} />
          </button>

          <div className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-dark-900 border border-slate-800 text-xs font-semibold text-slate-300">
            {isAdmin ? (
