  ...
  return (
    <div className="p-6 sm:p-10 space-y-8 max-w-7xl w-full mx-auto">
      ...
      <section aria-label="Resumen del inventario" className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-teal-200 bg-white p-5 shadow-sm"><p className="text-xs uppercase tracking-wider text-slate-600">Productos en catálogo</p><p className="mt-2 text-3xl font-black text-teal-800">{products.length}</p></div>
        <div className="rounded-2xl border border-teal-200 bg-white p-5 shadow-sm"><p className="text-xs uppercase tracking-wider text-slate-600">Unidades disponibles</p><p className="mt-2 text-3xl font-black text-teal-800">{products.reduce((total,p) => total+p.stock,0)}</p></div>
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 shadow-sm"><p className="text-xs uppercase tracking-wider text-amber-900">Categorías</p><p className="mt-2 text-3xl font-black text-amber-900">{new Set(products.map(p=>p.category)).size}</p></div>
      </section>
      {/* Main DataTable */}
      <DataTable
        products={products}
