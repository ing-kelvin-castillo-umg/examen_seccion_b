...
export const DataTable: React.FC<DataTableProps> = ({
  ...
  return (
    <div className="w-full bg-white rounded-3xl shadow-soft border border-ink-200 overflow-hidden">
      {/* Table Toolbar */}
      <div className="p-5 border-b border-ink-200 flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 bg-gradient-to-r from-white to-brand-50/50">
        {/* Search Bar */}
        <div className="relative w-full lg:w-96">
          ...
          <input
            ...
            className="w-full pl-10 pr-16 py-2.5 rounded-xl border border-ink-300 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-400 focus:border-transparent transition-all"
          />
          {searchTerm && (
            <button
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 w-full lg:w-auto sm:justify-end">
          ...
          {isAdmin ? (
            <button
              onClick={onCreate}
              className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold shadow-brand transition-all"
            >
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm text-slate-600">
          <thead className="bg-ink-900 text-ink-100 text-xs uppercase font-bold tracking-wider border-b border-ink-800">
            <tr>
          ...
          <tbody className="divide-y divide-ink-200">
            {filteredProducts.length > 0 ? (
              filteredProducts.map((product) => (
                <tr
                  key={product.id}
                  className="odd:bg-white even:bg-ink-50/60 hover:bg-brand-50 transition-colors group"
                >
                  {/* Name & Thumbnail */}
                    ...
                    <div className="inline-flex items-center gap-1.5">
                      ...
                      {isAdmin && (
                        <button
                          ...
                          className="p-2 text-ink-600 hover:text-amber-700 hover:bg-amber-50 rounded-lg transition-colors"
                        >
                      ...
                      {isAdmin && (
                        <button
                          ...
                          className="p-2 text-ink-600 hover:text-rose-700 hover:bg-rose-50 rounded-lg transition-colors"
                        >
