...
export const Carousel: React.FC<CarouselProps> = ({ products, onSelectProduct }) => {
  ...
  return (
    <div
      className="relative w-full overflow-hidden rounded-3xl bg-gradient-to-br from-ink-950 via-ink-800 to-brand-950 text-white shadow-2xl shadow-black/30 border border-brand-400/20"
    ...
    >
      ...
      <div className="relative min-h-[420px] sm:min-h-[460px] grid grid-cols-1 lg:grid-cols-12 items-center p-6 sm:p-10 gap-8">
        ...
        <div className="lg:col-span-6 flex flex-col justify-center space-y-4 z-10">
          ...
          <p className="text-ink-200 text-sm sm:text-base leading-relaxed line-clamp-3">
            {currentProduct.description}
          </p>

          <div className="pt-2 flex items-baseline gap-3">
            <span className="text-3xl sm:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-brand-300 via-cyan-200 to-sky-300">
          ...
          {onSelectProduct && (
            <div className="pt-2">
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-brand-400 hover:bg-brand-300 text-ink-950 font-bold text-sm shadow-brand transition-all hover:-translate-y-0.5 active:translate-y-0"
              >
      ...
      <button
        ...
        className="absolute left-3 top-1/2 -translate-y-1/2 p-2.5 rounded-full bg-ink-950/70 hover:bg-brand-500 text-white border border-white/15 backdrop-blur-md transition-all hover:scale-110 active:scale-95"
      >
      ...
      <button
        ...
        className="absolute right-3 top-1/2 -translate-y-1/2 p-2.5 rounded-full bg-ink-950/70 hover:bg-brand-500 text-white border border-white/15 backdrop-blur-md transition-all hover:scale-110 active:scale-95"
      >
