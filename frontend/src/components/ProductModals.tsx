...
export const ViewProductModal: React.FC<ViewProductModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink-950/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div role="dialog" aria-modal="true" aria-labelledby="view-product-title" className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl overflow-hidden border border-ink-200">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-ink-200 bg-gradient-to-r from-ink-50 to-brand-50">
          <div className="flex items-center gap-2">
            <span className="p-1.5 rounded-lg bg-brand-100 text-brand-700">
              <Tag className="w-4 h-4" />
            </span>
            <h3 id="view-product-title" className="text-lg font-bold text-ink-900">Detalle del Producto</h3>
          </div>
          <button
            onClick={onClose}
...
export const ProductFormModal: React.FC<ProductFormModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink-950/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div role="dialog" aria-modal="true" aria-labelledby="form-product-title" className="relative w-full max-w-xl bg-white rounded-3xl shadow-2xl overflow-hidden border border-ink-200">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-ink-200 bg-gradient-to-r from-ink-50 to-brand-50">
          <h3 id="form-product-title" className="text-lg font-bold text-ink-900">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-ink-300 focus:outline-none focus:ring-2 focus:ring-brand-400 focus:border-transparent text-sm"
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-ink-300 focus:outline-none focus:ring-2 focus:ring-brand-400 focus:border-transparent text-sm bg-white"
            >
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-ink-300 focus:outline-none focus:ring-2 focus:ring-brand-400 focus:border-transparent text-sm resize-none"
            />
          </div>
          ...
          <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="inline-flex items-center gap-2 px-5 py-2 text-sm font-semibold text-white bg-brand-600 hover:bg-brand-700 rounded-xl transition-colors disabled:opacity-50 shadow-brand"
            >
...
export const DeleteConfirmModal: React.FC<DeleteConfirmModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink-950/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div role="alertdialog" aria-modal="true" aria-labelledby="delete-product-title" className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 border border-ink-200">
        <div className="w-12 h-12 rounded-2xl bg-rose-100 text-rose-600 flex items-center justify-center mb-4">
          <AlertTriangle className="w-6 h-6" />
        </div>

        <h3 id="delete-product-title" className="text-lg font-bold text-slate-900 mb-2">¿Eliminar este producto?</h3>
